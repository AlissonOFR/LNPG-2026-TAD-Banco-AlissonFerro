...
print(f"Banco: {banco.nomeBanco}")
print(f"Código do Banco: {banco.codigo}")
print(f"Quantidade de Contas: {banco.quantidade_contas()}")
saldoTotal = 0
for conta in banco.listar_contas():
    print(f"Cliente: {conta.getTitular().getNome()}")
    print(f"CPF: {conta.getTitular().getCpf()}")
    print(f"Telefone: {conta.getTitular().getTelefone()}")
    print(f"Email: {conta.getTitular().getEmail()}")
    print(f"Conta: {conta.getNumero()}")
    print(f"Saldo: R$ {conta.consultarSaldo():.2f}")

    saldoTotal += conta.consultarSaldo()
print(f"Saldo total do banco: R$ {saldoTotal:.2f}")
